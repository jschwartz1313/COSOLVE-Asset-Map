...
The default `render.yaml` matches the current free web service and already-paid PostgreSQL database with automatic recovery. Its `start.sh` runs the release only after a successful build. The separate `render.production.yaml` creates paid web/database services and a daily source-monitoring job, with a dedicated pre-deploy release step. New resources incur charges; do not apply the separate template just to upgrade the existing service. See the deployment guide for backup retention, restore drills, and current hosting limitations.
...
Password recovery and email invitations require a configured mail provider. The current free Render web service blocks standard SMTP ports; it needs an HTTPS email backend or an approved hosting upgrade for SMTP. The reset page explains when email is not configured. Organization sign-in is enabled only when all three OIDC settings are present, and it accepts only active users that an administrator has already created. TOTP and recovery codes are available under **Account security**. In **Users and roles**, the **Send password setup or reset email** action provides the normal invitation workflow once email delivery is working.
