...
The checked-in catalog contains 494 real, publicly documented Virginia ecosystem assets updated on August 25, 2026. It is generated by `scripts/build_real_asset_catalog.py` and stored in `data/virginia_real_assets.json` so changes can be reviewed before they reach the database. A catalog update date is not an editorial-verification claim; reviewed records and source decisions are identified separately below.
