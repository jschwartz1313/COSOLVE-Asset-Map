...
The checked-in catalog contains 440 real, publicly documented Virginia ecosystem assets updated on August 21, 2026. It is generated by `scripts/build_real_asset_catalog.py` and stored in `data/virginia_real_assets.json` so changes can be reviewed before they reach the database. A catalog update date is not an editorial-verification claim; verified priority records are identified separately below.
...
Production deployments use `python manage.py seed_real_data --add-missing`, followed by `python manage.py enrich_asset_profiles` and `python manage.py apply_catalog_reviews`. The first command creates newly cataloged records and migrates explicitly mapped legacy catalog names without overwriting staff edits. The second fills blank profile and contact fields, adds missing public sources, and replaces only known obsolete catalog-generated website or contact links. The final command applies new checked-in editorial decisions once while preserving later staff review decisions. Other staff-maintained values remain authoritative.
