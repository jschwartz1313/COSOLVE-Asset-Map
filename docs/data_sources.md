...
The checked-in catalog contains 497 real, publicly documented Virginia ecosystem assets updated on August 30, 2026. It is generated by `scripts/build_real_asset_catalog.py` and stored in `data/virginia_real_assets.json` so changes can be reviewed before they reach the database. A catalog update date is not an editorial-verification claim; reviewed records and source decisions are identified separately below.
...
The `Manufacturing facilities` strategic category identifies 21 specific Virginia physical sites
...
fabrication plant, and Wrap Technologies' Norton manufacturing headquarters. Each new point uses
a published street address and official sources that establish both the site and its manufacturing
role. Micron is labeled as a supporting ecosystem asset because its Virginia fab serves multiple
markets, while official company material specifically identifies drone and autonomous-vehicle
applications for the products manufactured there.

Twenty category records are current or not known to be historical. RapidFlight's former Manassas
...
The three new editorial decisions are stored in
