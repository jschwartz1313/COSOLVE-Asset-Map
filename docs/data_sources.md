...
The checked-in catalog contains 390 real, publicly documented Virginia ecosystem assets updated on August 11, 2026. It is generated by `scripts/build_real_asset_catalog.py` and stored in `data/virginia_real_assets.json` so changes can be reviewed before they reach the database. A catalog update date is not an editorial-verification claim; verified priority records are identified separately below.
...
| Virginia degree-granting institutions with separately documented unmanned-systems connections | 19 | NCES IPEDS campus addresses and coordinates |
...
- Higher-education coverage includes only institutions with a separately documented unmanned-systems research, education, operations, facility, or program connection. NCES IPEDS provides institution and location details but is not treated as evidence of unmanned-systems relevance by itself.
...
The audit reviewed the primary website and attached public evidence for every catalog record. The filtered catalog contains 962 source attachments across 543 distinct URLs. Every one of the 390 primary website fields is also attached to its record as public evidence.
