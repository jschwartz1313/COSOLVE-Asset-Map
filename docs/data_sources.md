...
The checked-in catalog contains 525 Virginia ecosystem records updated on September 4, 2026, including one explicitly qualified research-follow-up listing. It is generated by `scripts/build_real_asset_catalog.py` and stored in `data/virginia_real_assets.json` so changes can be reviewed before they reach the database. A catalog update date is not an editorial-verification claim; reviewed records and source decisions are identified separately below.
...
| Curated research, workforce, company, public-safety, federal, program, and infrastructure assets | 362 | Official public addresses, named sites, or generalized locality coordinates |

The catalog covers 12 Virginia ecosystem regions and six record types. It contains 1,595 source attachments across 1,038 distinct URLs. There are 323 core unmanned-systems records and 202 supporting ecosystem records. These are catalog classifications, not a claim that each record has been freshly verified by a person. Blue Ridge Defense Works is retained with prominent research-follow-up qualifications and requires fresh editorial review.

There are 485 exact-address, site, campus, or FAA-reference points, 36 locality-level records, and four records without a single map point. The latter include regional programs/areas and PerimeterAI, whose Haymarket locality is documented but street address is not. Published phone or email coverage is 278 records; 140 have source-dated activity text. New geocoded business addresses use site precision: address-range interpolation is not a rooftop survey. Public offices used as references do not establish a drone launch point, equipment location, or the location of every organizational activity.
...
## September 4, 2026 expansion and audit

The pass added 25 records (19 core and six supporting), repaired eight source routes, added direct program evidence for Hampton University and Suffolk Police, upgraded six locality markers to documented public office/campus references, and corrected four region assignments to agree with the map boundaries. The FAA refresh also updated Warrenton/Fauquier reference coordinates at sub-meter rounding precision. The manufacturing-facility category now contains 26 records. Liquid Robotics is currently listed in Herndon but explicitly discloses its planned move to Maryland by the end of 2026.

Every one of the 525 records was included in the source and location audits. The final source sweep checked 1,038 unique URLs: 904 reachable, 108 blocked by the remote site, 23 network/TLS failures, one confirmed 404, and two reviewed homepage redirects. The source audit checks availability and screens visible text; it does not certify every claim. The older heuristic verification generator now produces pending review candidates only and cannot overwrite historical audit or review files.

All 483 street-address records were submitted to the Census geocoder: 401 returned a match (263 exact string matches and 138 non-exact matches), and 82 did not. Twelve matches differ from the catalog point by more than a kilometer, involving campus/airport/terminal reference points and a likely wrong-street Census match for Southwest Virginia Community College. These results were flagged rather than used to overwrite more appropriate source coordinates. Two airports use published reference coordinates without street addresses. Generalized and regional records retain their stated precision.

New records enter as source-backed listings; no automated HTTP check promotes them to human verification. Reviewed corrections use explicit old/new values, preserve staff edits and access controls, and retain obsolete evidence in database history. Full evidence and follow-up details are in [the audit report](audit_2026_09_04.md), [source findings](source_audit_2026_09_04.md), and [expansion research notes](research_candidates_2026_09_04.md).
...
## August 25, 2026 Hampton Roads expansion

A Hampton Roads-specific gap analysis added 21 source-backed records, increasing regional
coverage from 106 to 127 assets. The pass focused on capabilities that were underrepresented in
the earlier inventory: subsea robotics and services, maritime autonomy, Navy and NATO technology
transition, Army aviation experimentation, NASA flight research, counter-UAS training,
public-safety UAS, commercialization support, and civil surveying operations.

Every added point uses a current official public address or a documented host site. Office and
campus markers are explicitly distinguished from launch points, secure entrances, equipment
storage locations, or evidence that every published company capability operates in that building.
The Virginia Capes Range Complex remains a regional record without a point because a single dot
would misrepresent the large offshore and inshore operating area. Tidal Flight, G2 Ops, Hampton
Roads Helicopters, and REaKTOR are labeled as supporting assets where their documented role is
adjacent infrastructure, engineering, flight support, or commercialization rather than direct
proof of an uncrewed vehicle at the mapped address.

The pass deliberately excluded broad defense contractors without a specific public UxS claim,
organizations whose apparent Virginia location was actually out of state, and candidates without
a defensible Hampton Roads location. The 21 editorial decisions and 56 reviewed source links are
stored in `data/asset_editorial_reviews_2026_08_25_hampton_roads.json`. A final availability
sweep found no confirmed dead links: 43 URLs responded normally and 13 official defense or NATO
pages blocked the automated checker while remaining directly reviewable.

## August 30, 2026 manufacturing-facility review

The `Manufacturing facilities` strategic category identifies 25 specific Virginia physical sites
with public evidence of production, assembly, fabrication, or component manufacturing. It is
narrower than the existing `Manufacturing and supply chain` category: a headquarters, engineering
office, research center, incubator, or company-wide manufacturing claim does not qualify unless a
source connects manufacturing activity to the mapped Virginia site.

The review added Fulcrum Concepts' Newport News machine shop, Micron's Manassas semiconductor
fabrication plant, Wrap Technologies' Norton manufacturing headquarters, Defense Maritime
Solutions' Chesapeake facility, Radian Forge's Portsmouth facility, and L3Harris' Orange County
propulsion site. It also reclassified the existing Inertial Labs record after federal and company
sources tied manufacturing to its mapped Virginia establishment. Each new point uses a published
street address and sources that establish the site and its manufacturing role. Supporting sites
are explicitly distinguished from facilities with a dedicated UxS production line.

Twenty-four category records are current or not known to be historical. RapidFlight's former Manassas
manufacturing headquarters remains in the category as a clearly labeled historical record so the
inventory does not erase documented past capacity. Heven AeroTech's Winchester manufacturing
campus remains locality-level because no reviewed official source publishes a street address.
StewTech remains excluded because the reviewed public sources conflict on location and do not
establish the claimed plant address. Avio USA's announced Hurt factory remains excluded from the
narrow category until an operating production site and a defensible UxS connection are documented.
The seven reviewed editorial decisions and two exclusions are stored in
